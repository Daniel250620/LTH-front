...
import { User, Phone, Truck, CreditCard, MessageCircle } from "lucide-react";
   ...
   <div className="space-y-4">
    <div>
     ...
     {customer?.client_number && (
      <a
       href={`tel:${customer.client_number}`}
       className="inline-flex items-center gap-1.5 text-slate-400 hover:text-[#102B5E] text-sm pt-1 font-bold transition-colors"
      >
    ...
    {customer?.id && (
     <>
      ...
      <Link
       href={`/chat?customerId=${customer.id}`}
       className="relative w-full flex items-center justify-center gap-2.5 bg-gradient-to-r from-[#102B5E] to-[#1a3d7a] text-white py-3.5 px-4 rounded-xl font-black text-xs uppercase tracking-wider overflow-hidden shadow-md hover:shadow-lg hover:shadow-blue-900/10 transition-all duration-300 active:scale-[0.98] group/btn"
      >
       <div className="absolute top-0 -left-full w-1/2 h-full bg-white/15 transform -skew-x-12 transition-none group-hover/btn:animate-shine-btn" />
       <style>{`
                @keyframes shine-effect {
                  100% { left: 125%; }
                }
                .group\\/btn:hover .group-hover\\/btn\\:animate-shine-btn {
                  animation: shine-effect 0.85s ease-out;
                }
              `}</style>
       <MessageCircle
        size={15}
        className="group-hover/btn:scale-125 transition-transform duration-300 text-emerald-400 fill-emerald-400/20"
       />
       <span>Ir a Conversación</span>
       <span className="absolute top-4.5 right-4 flex h-2 w-2">
