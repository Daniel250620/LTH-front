...
export default function WhatsAppDocument({
  ...
  const handleDownloadOrOpen = async (action: "open" | "download", e: React.MouseEvent) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  return (
    ...
    >
      ...
      >
        <FileText
          size={22}
          className={isOut ? "text-red-300" : "text-red-500"}
        />
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          ...
          {error && <span className="ml-2 text-red-400 normal-case">Error al descargar</span>}
        </p>
      </div>
      ...
      <div className="flex items-center gap-1 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
        {loading ? (
          <span className="p-1.5 rounded-lg">
            <Loader2 size={16} className={`animate-spin ${isOut ? "text-white/70" : "text-zinc-500"}`} />
          </span>
        ) : (
          <>
            <span
