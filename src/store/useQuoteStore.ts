...
export const useQuoteStore = create<QuoteStore>((set, get) => ({
 ...
 fetchQuoteById: async (id) => {
  // Si está cargando o ya tenemos el dato, no repetir el fetch innecesariamente
  const { loading, selectedQuote } = get();
  if (loading && selectedQuote?.id === id) return;

  set({ loading: true, error: null });
  try {
   const response = await fetch(`${QUOTE_API_URL}/${id}`);
   if (!response.ok) {
    throw new Error("No se pudo cargar la cotización");
   }
   const data = await response.json();
   set({ selectedQuote: data, loading: false });
  } catch (error) {
   set({
    error: (error as Error).message || "Error al obtener la cotización",
    loading: false,
   });
  }
 },
