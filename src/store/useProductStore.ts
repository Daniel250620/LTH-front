...
import { VehicleCompatibility } from "@/components/CompatibleVehiclesModal";
...
interface ProductStore {
 ...
 fetchVehiclesByProduct: (id: number) => Promise<VehicleCompatibility[]>;
...
export const useProductStore = create<ProductStore>((set, get) => ({
 ...
 fetchProducts: async (params = {}) => {
  ...
  try {
   ...
   set({
    error:
     (error as Error).message || "Ocurrió un error al cargar los productos",
    loading: false,
   });
  }
 },
 ...
 fetchVehiclesByProduct: async (id: number) => {
  try {
   const response = await fetch(`${API_BASE_URL}/${id}/vehicles`);
   if (!response.ok) {
    if (response.status === 404) {
     return [];
    }
    console.error("No se pudo obtener la compatibilidad");
    return [];
   }
   const vehicles: VehicleCompatibility[] = await response.json();
   return vehicles;
  } catch (error) {
   console.error("Error al obtener la compatibilidad", error);
   return [];
  }
 },
